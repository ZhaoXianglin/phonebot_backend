...
import pprint
import copy
from utils.function import helper
from utils.function.tool import time_helper
...
def filter_items_by_user_constraints(user_constraints, item_pool, minimal_threshold, categorical_attributes,
...
def compute_recommendation_by_MAUT(user_preference_model, item_pool, top_K, categorical_attributes,
                                   numerical_attributes, key, sort=True):
    ...
    for each_item in item_pool:
        ...
        for attr in categorical_attributes:
            user_item_preference_value_dict[attr] = categorical_attributes_value_function(
                user_pref_preference_value[attr], each_item[attr], attr)
        # 2. Numerical Attributes
        for attr in numerical_attributes:
            user_item_preference_value_dict[attr] = numerical_attributes_value_function(
                user_pref_preference_value[attr], each_item[attr], attr)
...
def update_based_on_satisfiability(attr, satisfiability, satisfied_critique_attribute_list,
...
def compute_recommendation_compatibility_score(user_critique_preference, item_pool, top_K, categorical_attributes,
                                               numerical_attributes, key, sort=True):
...
def compute_recommendation(user_preference_model, user_critique_preference, item_pool, top_K, categorical_attributes,
                           numerical_attributes, method, key, alpha=0.5, sort=True):
    if len(item_pool) < top_K:
        top_K = len(item_pool)
        # print(top_K)
    if method == 'MAUT':
        top_K_recommmendation_list = compute_recommendation_by_MAUT(user_preference_model, item_pool, top_K,
                                                                    categorical_attributes, numerical_attributes, key,
                                                                    sort)
        return top_K_recommmendation_list

    if method == 'COMPAT':
        top_K_recommmendation_list = compute_recommendation_compatibility_score(user_critique_preference, item_pool,
                                                                                top_K, categorical_attributes,
                                                                                numerical_attributes, key, sort)
        return top_K_recommmendation_list

    if method == 'MAUT_COMPAT':
        item_maut_score_dict = compute_recommendation_by_MAUT(user_preference_model, item_pool, len(item_pool),
                                                              categorical_attributes, numerical_attributes, key,
                                                              sort=False)
        item_compatibility_score_dict = compute_recommendation_compatibility_score(user_critique_preference, item_pool,
                                                                                   top_K, categorical_attributes,
                                                                                   numerical_attributes, key,
                                                                                   sort=False)
...
def update_recommendation_pool(user_preference_model, user_critique_preference, new_item_pool, integrated_item_pool,
